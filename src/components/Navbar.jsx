        ...
        <>
            ...
            <nav className="bg-[rgba(30,30,30,0.8)] backdrop-blur-lg p-4 fixed top-0 left-0 right-0 z-50 flex justify-between items-center shadow-lg">
                ...
                <div className="flex items-center">
                    <img className="w-10 h-10 rounded-full mr-4 transition-transform duration-300 ease-in-out hover:scale-110" src={logo} alt="Logo" />
                    <a href="#home" className="text-red-500 font-dm-sans text-2xl font-bold tracking-wide">Club Gamma</a>
                </div>

                {/* Navbar Links (visible on larger screens) */}
                <div className="hidden lg:flex gap-12 font-montserrat text-lg">
                    <a href="#home" className="text-white hover:text-red-500 transition-all duration-300">Home</a>
                    <button onClick={onQandAClick} className="text-white hover:text-red-500 transition-all duration-300">Q&A</button>
                    <button onClick={onStatusClick} className="text-white hover:text-red-500 transition-all duration-300">Stats</button>
                    <button onClick={onProjectsClick} className="text-white hover:text-red-500 transition-all duration-300">Projects</button>
                    <button onClick={onContactClick} className="text-white hover:text-red-500 transition-all duration-300">Contact</button>
                </div>
                ...
                <div className="hidden lg:flex">
                    {Global.user ? (
                        ...
                        <button onClick={handleGithubLogin} className="flex font-dm-sans items-center bg-[#181717] text-white py-2 px-5 rounded-md font-bold shadow-lg hover:shadow-none transition-all duration-500">
            ...
            >
                <div className="p-4">
                    ...
                    <nav className="space-y-6">
                        <a href="#home" className="text-white block">Home</a>
                        ...
                        {Global.user ? (
                            <>
                                <button className="text-white block">Profile</button>
                                <button className="text-white block">Settings</button>
                                <button onClick={handleLogout} className="text-white block">Logout</button>
                            </>
                        ) : (
                            <Button onClick={handleGithubLogin} className="text-white bg-red-500 block"> Continue with GitHub</Button>
                        )}
                    </nav>
                </div>
            </div>
